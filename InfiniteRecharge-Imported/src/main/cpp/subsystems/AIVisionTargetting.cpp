...
char AIVisionTargetting::GetSideOfIntakeWithTargetFromMainCam() {
    ...
    if (angFromIntakeCenter < 80 || servoPan < 85)
        return 'r';
    else if (angFromIntakeCenter > 105)
        return 'l';
    else
        return 'c';
}
...
// This does not currently seem to work as expected
char AIVisionTargetting::GetPrecisePowercellSideOfIntake() {
    bool wasSweeping = RobotContainer::cameraMount->GetIsSweeping();
    if (wasSweeping)
